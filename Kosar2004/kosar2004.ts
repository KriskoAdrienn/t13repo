const eredmenyek: string[] = ["7up Joventut;Adecco Estudiantes;81;73;Palacio Mun. De Deportes De Badalona;2005-04-03",
    ...
    function AdatFeltolto(feltoltendoElem: string[]):kosarAdat[]{
        let beolvasottAdatok: kosarAdat[] =[];
        for (let i: number=0; i<feltoltendoElem.length; i++){
            let daraboltAdatok = feltoltendoElem[i].split(";");
            let ujCsapat:kosarAdat = {
    ...
    var kosarAdatok: kosarAdat[] = AdatFeltolto(eredmenyek)
    console.log(kosarAdatok)

    //3. feladat

    function RealMadrid(vizsgaltTomb:kosarAdat[]):[number, number]{
        let hazaiMerkozes:number=0;
        let idegenMerkozes:number=0;
        for (let i: number=0;i<vizsgaltTomb.length;i++){
                if (vizsgaltTomb[i].hazai=="Real Madrid"){
                    hazaiMerkozes++
                }else if(vizsgaltTomb[i].idegen=="Real Madrid"){
                    idegenMerkozes++
                }
        } 
       return [hazaiMerkozes,idegenMerkozes]
    }
    ...
    RealMadridKiir(RealMadrid(kosarAdatok))

    //4. feladat

    function dontetlenMerkozes(vizsgaltTomb:kosarAdat[]):boolean{
        let voltEDontetlen:boolean=false;
        for(let i:number=0;i<vizsgaltTomb.length;i++){
            if (vizsgaltTomb[i].hazai_pont==vizsgaltTomb[i].idegen_pont){
                voltEDontetlen=true
            }
        }
        return voltEDontetlen
    }
    function dontetlenMerkozesKiir(eredmeny:boolean):void{
        console.log("4. feladat: volt döntetlen? "+eredmeny)
    }

    dontetlenMerkozesKiir(dontetlenMerkozes(kosarAdatok))

    //5. feladat

    function BarcelonaCsapatNeve(vizsgaltTomb:kosarAdat[]):string{
        let csapatNev:string="";
        for (let i:number=0;i<vizsgaltTomb.length;i++){
            if (vizsgaltTomb[i].hazai.match("Barcelona")){
                csapatNev = vizsgaltTomb[i].hazai
            }
        }
        return csapatNev
    }
    
    function CsapatNeveKiir(nev:string): void {
        console.log("5. feladat:barcelonai csapat neve "+nev);
    }
    
    CsapatNeveKiir(BarcelonaCsapatNeve(kosarAdatok))
    ...
    function Noveber2004(vizsgaltTomb:kosarAdat[]):November2004[]{
        let csapatInfo: November2004[]=[]
        for(let i: number = 0; i < vizsgaltTomb.length; i++){
            if(vizsgaltTomb[i].idopont=="2004-11-21"){
                let ujCsapat:November2004={
                    hazaiCsapat:vizsgaltTomb[i].hazai,
                    idegenCsapat:vizsgaltTomb[i].idegen,
                    hazaiPont: vizsgaltTomb[i].hazai_pont,
                    idegenPont: vizsgaltTomb[i].idegen_pont
                }
    ...
    function Noveber2004Kiir(kiirandoEredmenyek:November2004[]):void{
    ...
    Noveber2004Kiir(Noveber2004(kosarAdatok))
    ...
    function stadionStatisztika(vizsgaltTomb:kosarAdat[]):Statisztika[]{
        let statisztikaAdat:Statisztika[]=[];
        for(let i: number = 0; i < vizsgaltTomb.length; i++){
            let szerepelE:boolean=false;
            let adottIndex:number=0;
            for(let j:number=0;j<statisztikaAdat.length;j++){
                if(vizsgaltTomb[i].helyszin==statisztikaAdat[j].stadionNeve){
                    szerepelE=true;
                    adottIndex = j;
                }
            }
            if(!szerepelE){
                let ujStadion:Statisztika={stadionNeve:vizsgaltTomb[i].helyszin, merkozesSzam:1}
                statisztikaAdat.push(ujStadion)
            }
            else{
                statisztikaAdat[adottIndex].merkozesSzam++
            }
        }
        return statisztikaAdat
    }
    
    function stadionStatisztikaKiir(eredmenyObjektum:Statisztika[]):void{
        for(let i: number = 0; i < eredmenyObjektum.length; i++){
            if(eredmenyObjektum[i].merkozesSzam>20){
                console.log("7.feladat: "+eredmenyObjektum[i].stadionNeve+ ": "+ eredmenyObjektum[i].merkozesSzam)
            }
        }
    }
